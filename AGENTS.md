...
- 当用户声明「这是一次版本更新」，或该轮改动大到足以当作一次更新（跨多模块、有面向用户的新功能/行为变化等，判定标准见该 skill）时，在**根目录 `更新日志.md`** 追加一节更新日志：正序追加，最新小节在文件末尾，同日并入末尾小节，不改写历史条目。
- `修改记录.md` 是历史变更记录，**保持只读**：不追加、不改写、不删除。
